...
export default function AIPanel({ data }) {
  ...
  const Step = ({ icon, label, color, bg, text }) => (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        <div style={{ width: 22, height: 22, borderRadius: 6, background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11 }}>{icon}</div>
        <span style={{ fontSize: 12, fontWeight: 600, color }}>{label}</span>
      </div>
      <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6, paddingLeft: 30 }}>{text}</div>
    </div>
  );
    ...
    <div style={{ background: 'var(--surface)', border: '1px solid rgba(108,143,255,0.25)', borderRadius: 14, padding: 20, marginBottom: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <button onClick={analyze} style={{ background: 'rgba(108,143,255,0.1)', border: '1px solid rgba(108,143,255,0.3)', borderRadius: 8, padding: '5px 14px', color: '#6c8fff', fontSize: 12, cursor: 'pointer' }}>
          {loading ? 'Анализирую...' : 'Обновить данные'}
        </button>
      </div>

      {loading && <div style={{ color: 'var(--text-tertiary)', fontSize: 13, textAlign: 'center', padding: 20 }}>AI анализирует данные производства...</div>}
      {error && <div style={{ color: '#E24B4A', fontSize: 13, padding: 10 }}>{error}</div>}

      {analysis && !loading && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div>
            {analysis.bottlenecks?.map((b, i) => (
              <div key={i} style={{ padding: 14, background: 'rgba(255,255,255,0.03)', borderRadius: 10, marginBottom: 10 }}>
          ...
          <div>
            {analysis.priorities && (
              <div style={{ padding: 14, background: 'rgba(255,255,255,0.03)', borderRadius: 10, marginBottom: 10 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 12 }}>Приоритетные действия</div>
                {analysis.priorities.map((p, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: '#6c8fff', width: 18 }}>{p.rank}</span>
                    <span style={{ fontSize: 12, color: 'var(--text-secondary)', flex: 1, lineHeight: 1.4 }}>{p.action}</span>
                    <span style={{ fontSize: 11, color: '#4ecda4', fontWeight: 600, whiteSpace: 'nowrap' }}>{p.expected_oee_gain}</span>
                    <span style={{
            ...
            {analysis.summary && (
              <div style={{ padding: 14, background: 'rgba(255,255,255,0.03)', borderRadius: 10 }}>
                <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 6 }}>Полный отчёт AI</div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6 }}>{analysis.summary}</div>
              </div>
            )}
          </div>
        </div>
